...
#ifdef  AVILIB_EXPORTS
#define AVILIB_API __declspec(dllexport)
#else
#define AVILIB_API __declspec(dllimport)
#endif
...
AVILIB_API avi_t *AVI_open_input_file(const char *filename, int getIndex);
AVILIB_API int  AVI_close(avi_t *AVI);

AVILIB_API long AVI_video_frames(avi_t *AVI);
AVILIB_API int  AVI_video_width(avi_t *AVI);
AVILIB_API int  AVI_video_height(avi_t *AVI);
AVILIB_API double AVI_video_frame_rate(avi_t *AVI);
AVILIB_API char* AVI_video_compressor(avi_t *AVI);

AVILIB_API long AVI_frame_size(avi_t *AVI, long frame);
AVILIB_API int  AVI_seek_start(avi_t *AVI);
AVILIB_API int  AVI_set_video_position(avi_t *AVI, long frame, long *frame_len);
AVILIB_API long AVI_read_frame(avi_t *AVI, char *vidbuf);

AVILIB_API int  AVI_read_data(avi_t *AVI, char *vidbuf, long max_vidbuf,
